<div id="editGuardiansModal-{{ $guardian->id }}" tabindex="-1" aria-hidden="true"
    class="hidden fixed inset-0 z-50 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-black/20">
    ...
    <div class="relative w-full max-w-2xl p-4">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg relative">
            ...
            <button type="button"
                ...
                data-modal-hide="editGuardiansModal-{{ $guardian->id }}">
                ✕
            </button>

            <!-- Modal Content -->
            <div class="p-6 space-y-5">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Guardian</h3>

                <form method="POST" action="{{ route('admin.guardians.update', $guardian->id) }}"
                    onsubmit="document.getElementById('editGuardiansModal-{{ $guardian->id }}').classList.add('hidden')"
                    class="space-y-4">
                    ...
                    <div class="grid gap-4 mb-6 sm:grid-cols-2">
                        ...
                        {{-- JOB --}}
                        <x-admin.field label="Job" name="job" required
                            value="{{ old('job', $guardian->job) }}" />

                        {{-- PHONE --}}
                        <x-admin.field label="Phone" type="number" name="phone" required
                            value="{{ old('phone', $guardian->phone) }}" />

                        {{-- EMAIL --}}
                        <x-admin.field label="Email" name="email" type="email" required
                        ...
                        <div>
                            ...
                            <select name="gender"
                                ...
                                <option value="Female" {{ old('gender', $guardian->gender) == 'Female' ? 'selected' : '' }}>
                                    Female
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="address" rows="4"
                                ...
                                focus:ring-primary-500 focus:border-primary-500
                                dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('address', $guardian->address) }}</textarea>
                        </div>
                    </div>
